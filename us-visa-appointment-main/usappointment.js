...
(async () => {
 ...
 await log("run");
      ...
      {
          const targetPage = page;
          const ap=await targetPage.goto('https://ais.usvisa-info.com/en-' + region + '/niv/users/sign_in', { waitUntil: 'domcontentloaded' });
      ...
      // We are logged in now. Check available dates from the API
      {
          ...
          if (firstDate > currentDate) {
			...
			log("There is not an earlier date available than " + firstDate.toISOString().slice(0,10));
            await browser.close();
            return false;
          }
